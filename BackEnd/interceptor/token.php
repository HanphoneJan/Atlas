...
function createToken($user_email, $username, $is_admin) {
    ...
    $payload = json_encode([
        'iss' => getenv('JWT_ISSUER') ?: 'your_issuer',  // 从环境变量获取签发者
        'aud' => getenv('JWT_AUDIENCE') ?: 'your_audience',  // 从环境变量获取受众
        'iat' => time(),
...
function verifyToken($token) {
    ...
    // 可选：验证签发者和受众
    if ($payload['iss'] !== getenv('JWT_ISSUER') && $payload['iss'] !== 'your_issuer') {
        return false;
    }
    if ($payload['aud'] !== getenv('JWT_AUDIENCE') && $payload['aud'] !== 'your_audience') {
        return false;
    }
